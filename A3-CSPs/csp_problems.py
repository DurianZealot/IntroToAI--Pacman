...
def solve_planes(planes_problem, algo, allsolns,
    ...
    def initPlaneCSP(planes_problem):
        ...
        for planeIndex in range(len(planes)):
            ...
            for flyableFlightIndex in range(len(planes_can_fly_flights[planes[planeIndex]])):
                newVar = Variable("{},{}".format(planes[planeIndex], planes_can_fly_flights[planes[planeIndex]][flyableFlightIndex]), domain)   
        ...
        for sub_arr in variable_arr:
            for index in range(len(sub_arr) - min_maintenance_frequency + 1):
                ...
                constraint_list.extend([NValuesConstraint("[{}] maintenance flight".format(sub_arr), scope, required_values, 1, min_maintenance_frequency)])
        ...
        for sub_arr in variable_arr:
            for index in range(len(sub_arr) - 1):
                ...
                new_constraint = TableConstraint("[{}] consecutive flights".format(sub_arr), [prevFlight, nextFlight], allConsecutiveFlights)
        ...
        for index in range(len(variable_arr)):
            ...
            constraint_list.extend([TableConstraint("[{}] first flight".format(initialFlight), [initialFlight], initialFlightDomain)])
        ...
        constraint_list.extend([coverAndNoDuplicate("all flights", variables, flights)])
