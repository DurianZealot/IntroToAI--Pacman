...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        # type(qi) --> Variable
        # variable.domain() is the possible position in column
        for x in qi.domain():
            # iterate the domain of the first variable
            for y in qj.domain():
                # iterate the domain of the second variable
                if x != y and abs(x-y) != abs(i-j):
                    # constraints : not in the same column, not in the same diagonal
                    satisfyingAssignments.append([x,y])
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        for var in self.scope():
            ...
            if var.isAssigned():
                ...
                if var.getValue() in self._required:
                    qualifiedNum += 1
        return self._lb <= qualifiedNum <= self._ub 
    ...
    def hasSupport(self, var, val):
        ...
        def satisfyBounds(assigments):
            qualifiedNum = 0
            for (var, val) in assigments:
        ...
        return findvals(varsToAssign, [(var, val)], satisfyBounds)
        
class coverAndNoDuplicate(Constraint):
    ...
    def check(self):
        carriedFlights = set()
        for planeVar in self.scope():
            if planeVar.isAssigned():
                flight = planeVar.getValue()
                if flight in carriedFlights:
                    return False
                else:
                    carriedFlights.add(flight)
        return len(carriedFlights) == self._flights
    ...
    def hasSupport(self, var, val):
        ...
        def canCoverAllAndNoDuplicate(assigments):
            carriedFlights = set()
            for (planeVar, flight) in assigments:
                if flight != 0 :
                    if flight in carriedFlights:
                        return False
                    else:
                        carriedFlights.add(flight)
            return len(carriedFlights) == len(self._flights)

        
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        return findvals(varsToAssign, [(var, val)], canCoverAllAndNoDuplicate)