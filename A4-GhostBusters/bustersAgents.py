...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # Find the most likely position of each ghosts uncaptured
        maxProb = 0 
        maxPosition = None 

        for ghostBeliefs in livingGhostPositionDistributions:
            for posisition in ghostBeliefs:
                if ghostBeliefs[posisition] > maxProb:
                        maxProb = ghostBeliefs[posisition]
                        maxPosition = posisition

        minPosition = None
        minDistance = 9999999999999
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            currDistance = self.distancer.getDistance(successorPosition, maxPosition)
